...
contract GodCoin is Context, IERC20, Ownable {
    ...
    address payable public charityAddress = payable(0x5C568A9db969E20fF65bE62F9ee0b8017E42780e); // Charity Address
    ...
    string private _name = "God"; // token name
    string private _symbol = "GOD"; // token symbol
    uint8 private _decimals = 9; // 1 token can be divided into 10e_decimals parts
    
    uint256 public _charityFee = 4; // charity fee
    ...
    uint256 public _reflectionFee = 2; //reflection fee
    uint256 private _previousReflectionFee = _reflectionFee; //reflection fee
    
    uint256 private _taxFee = _burnFee.add(_reflectionFee); // burn+reflection
    uint256 private _previousTaxFee = _taxFee; // restore old tax fee
    
    uint256 private _liquidityFee = _charityFee.add(5); // liquidity + charity fee on each transaction
    uint256 private _previousLiquidityFee = _liquidityFee; // restore old liquidity fee
    ...
    uint256 public _maxTxAmount = 5000000 * 10**6 * 10**9; // max allowed tokens tranfer per transaction
    uint256 private numTokensSellToAddToLiquidity = 500000 * 10**6 * 10**9; // min token liquidity fee collected before swapandLiquify
    ...
    constructor () {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); // TO-DO Chnage Adress While Deploying
    ...
    /**  
     * @dev return's correct liquidity fee
     */
    function getliquidityFee() public view returns (uint256)
    {
        return _liquidityFee.sub(_charityFee);
    }
    ...
    function setCharityFeePercent(uint256 Fee) external onlyOwner() {
        _liquidityFee = _liquidityFee.sub(_charityFee);
        _charityFee = Fee;
        _liquidityFee = _liquidityFee.add(_charityFee);
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _charityFee == 0 && _reflectionFee == 0) return;
     ...
     * before swaped and BNB send to charity wallet
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if(_liquidityFee.sub(_charityFee) > 0)
        {
